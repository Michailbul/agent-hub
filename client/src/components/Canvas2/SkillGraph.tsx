...
export function SkillGraph() {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden relative bg-[#141413]" ref={containerRef}>
      {/* Controls */}
      <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
        <div>
          <input
            className="w-52 px-3 py-1.5 font-mono text-[11px] text-[rgba(227,224,217,0.75)] bg-[rgba(255,255,255,0.06)] border-2 border-[rgba(255,255,255,0.12)] outline-none transition-all duration-[80ms] placeholder:text-[rgba(255,255,255,0.25)] focus:border-[rgba(217,119,87,0.5)] focus:shadow-[0_0_0_1px_rgba(217,119,87,0.2)]"
            type="text"
        ...
        <div className="flex flex-wrap gap-1.5">
          {depts.map(d => (
            <div key={d} className="flex items-center gap-1.5">
              <span className="w-2 h-2" style={{ background: DEPT_COLORS[d] || DEFAULT_COLOR }} />
              <span className="font-mono text-[9px] text-[rgba(227,224,217,0.5)]">{d}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <canvas
        ref={canvasRef}
        className="flex-1 w-full h-full cursor-grab"
        onMouseMove={onMove}
        onMouseDown={onDown}
        onMouseUp={onUp}
        onMouseLeave={onUp}
      ...
      <div className="absolute bottom-4 right-4 z-10">
        <span className="font-mono text-[10px] text-[rgba(227,224,217,0.4)]">
          {gd.nodes.length} nodes &middot; {gd.links.length} edges
        </span>
      </div>
    </div>
  )
